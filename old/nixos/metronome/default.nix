...
let
  ...
  metronome = pkgs.writeShellScriptBin "metronome.sh" ''
    ...
    beat_time="$(${pkgs.bc}/bin/bc -l <<< "scale=5; 60/$bpm-0.004")"
    ...
    while true; do
        for ((i=1; i<=$msr; i++)); do
            if [[ $i -eq 1 ]]; then
                ...
                ${pkgs.mpv}/bin/mpv --no-config --keep-open=no --force-window=no --no-input-terminal --quiet $tick &>/dev/null &
            else
                # Unaccentuated beat
                # canberra-gtk-play --id="$tock" &
                ${pkgs.mpv}/bin/mpv --no-config --keep-open=no --force-window=no --no-input-terminal --quiet $tock &>/dev/null &
            fi
